...
- tap
##### tap
Для выполнения первого задания с помощью vpn интерфейса tap достаточно развернуть vagrant-стенд, который автомоатически развернет две ВМ с имитацией VPN-тунеля между двумя ПК находящимся в разных сторонах глобальной сети.
```
...
##### tun
По аналогичному сценарию можно запустить тот же стенд для настройки vpn-туннеля через tun-интерфейсы, с одним только изменением, необходимо заменить первую строку в двух файлах server.conf и client.conf, ```dev tap``` на ```dev tun```.
